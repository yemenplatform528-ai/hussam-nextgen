...
def upgrade():
    ...
    if 'payment_rail_registry' not in tables:
        op.create_table(
    ...
    if 'payment_adapter_registry' not in tables:
        op.create_table(
    ...
    if 'payment_adapter_registry' in set(sa.inspect(bind).get_table_names()):
        indexes = {i['name'] for i in sa.inspect(bind).get_indexes('payment_adapter_registry')}
        if 'ix_payment_adapter_registry_provider_rail' not in indexes:
