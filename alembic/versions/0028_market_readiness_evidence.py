...
def upgrade():
    ...
    if 'market_readiness_evidence' not in set(insp.get_table_names()):
        op.create_table(
    ...
    indexes = {i['name'] for i in sa.inspect(bind).get_indexes('market_readiness_evidence')}
    if 'ix_market_readiness_evidence_market_requirement' not in indexes:
