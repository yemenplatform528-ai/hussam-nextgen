...
A machine-readable cross-check was also completed against the full GitHub blob for `YemenOpenSource/Yemen-info` (`yemen-info.json`, blob `30c22263c57022bed8bf511c1b2f975164897bb1`) and the full candidate district blob from `open-admin-data/yemen-administrative-divisions` (`all-district.json`, blob `e884e01f4d7e261469e3344bd54783f09ca766af`). Parsing the complete independent JSON yields 22 governorates and 335 districts. The candidate contains 335 district records, 335 unique district IDs, and all 335 candidate district records reference a parent governorate ID that is present in the candidate district parent set. This is a structural cross-check only: the independent dataset does not expose the same OCHA P-code key in the inspected district records, so this does not substitute for an OCHA P-code/artifact comparison.
