...
def test_complete_business_flow_from_purchase_to_settlement():
    ...
    with Factory() as db:
        ...
        db.add(provider); db.flush(); db.add_all([ProviderMarketCapability(provider_id=provider.id, market_id=market.id, capability='payment', currency='YER', active=True), ProviderMarketCapability(provider_id=provider.id, market_id=market.id, capability='settlement', currency='YER', active=True)]); db.flush(); rail = PaymentRailRegistryEntry(market_id=market.id, code='wallet', capability='payment', currency='YER', status='production'); db.add(rail); db.flush(); db.add(PaymentAdapterRegistryEntry(provider_id=provider.id, rail_id=rail.id, adapter_code='test.wallet', adapter_version='1.0.0', status='production', active=True)); db.commit()
