...
def test_coupon_redemption_is_bound_to_order_seller_tenant():
    ...
    db.add(order); db.commit(); db.refresh(order)
    svc.create_coupon(other_seller.id,'SHARED-CODE','percentage',Decimal('10'),now-timedelta(minutes=1),now+timedelta(hours=1),currency='YER',minimum_subtotal=500)
    with pytest.raises(MarketplaceCompletionError, match='coupon not found'):
